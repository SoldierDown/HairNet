...
from viz import visualize_hair
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=2, mode='bilinear',
                          align_corners=False)  # (batch_size, 512, 8, 8)
        x = F.relu(self.conv7(x))  # (batch_size, 512, 8, 8)
        x = F.interpolate(x, scale_factor=2, mode='bilinear',
                          align_corners=False)  # (batch_size, 512, 16, 16)
        x = F.relu(self.conv8(x))  # (batch_size, 512, 16, 16)
        x = F.interpolate(x, scale_factor=2, mode='bilinear',
                          align_corners=False)  # (batch_size, 512, 32, 32)
...
class MyLoss(nn.Module):
    ...
    def forward(self, output, convdata, visweight):
        ...
        for i in range(0, 32):
            for j in range(0, 32):
                pos_loss += (visweight[:, :, i, j].reshape(1, -1).mm(torch.pow(
                    (convdata[:, :, 0:3, i, j]-output[:, :, 0:3, i, j]), 2).reshape(-1, 3))).sum()
                cur_loss += (visweight[:, :, i, j].reshape(1, -1).mm(torch.pow(
                    (convdata[:, :, 3, i, j]-output[:, :, 3, i, j]), 2).reshape(-1, 1))).sum()
...
class MyPosEvaluation(nn.Module):
    ...
    def forward(self, output, convdata):
        ...
        for i in range(0, 32):
            for j in range(0, 32):
                loss += torch.mean(torch.abs(convdata[:,
                                   :, 0:3, i, j]-output[:, :, 0:3, i, j]))
        return loss/1024.0
...
def train(root_dir, weight_path):
    ...
    train_data = HairNetDataset(
        project_dir=root_dir, train_flag=1, noise_flag=1)
    ...
    for i in range(EPOCH):
        ...
        if (i+1) % WEIGHT_STEP == 0:
            save_path = root_dir + '/weight/' + \
                str(i+1).zfill(6) + '_weight.pt'
...
def test(root_dir, weight_path):
    ...
    test_data = HairNetDataset(
        project_dir=root_dir, train_flag=0, noise_flag=0)
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        # convdata shape:   [BATCH_SIZE, 100, 4, 32, 32]
        # visweight shape:  [BATCH_SIZE, 100, 32, 32]
        
        img_cuda = img.cuda()
        convdata_cuda = convdata.cuda()
        visweight = visweight.cuda()
        output_cuda = net(img_cuda)
        # output shape:     [BATCH_SIZE, 100, 4, 32, 32]
        pos = pos_error(output_cuda, convdata_cuda)
        cur = cur_error(output_cuda, convdata_cuda)
        print(str(BATCH_SIZE*(i+1)) + '/' + str(len(test_data)) + ', Position loss: ' +
              str(pos.item()) + ', Curvature loss: ' + str(cur.item()))
        
        visualize_hair(str(BATCH_SIZE*(i+1)), img, convdata[:, :, 0:3, :, :], output_cuda.cpu().detach().numpy()[:, :, 0:3, :, :])
